...
def test_flash_infer_batch_decode(
    ...
    k_seq_lens = [random.randint(1, 10) for _ in range(batch_size)]
    ...
    kv_last_page_len = torch.tensor([k_seq_len % page_size for k_seq_len in k_seq_lens], dtype=torch.int, device=device)
