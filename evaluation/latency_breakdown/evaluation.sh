...
for rate in 3 6 9 12 15 18 21 24 27 30; do
    ...
    conda run -n dxz_dev --no-capture-output python benchmark.py --num-requests=100 --backend=dxz --inference-mode=online --model-path=/mnt/cfs/9n-das-admin/llm_models/llava-1.5-7b-hf --textcaps=1 --port=8888 --log-request --log-output --request-rate ${rate}
    # > /dev/null 2>&1
    ...
    conda run -n dxz_dev --no-capture-output python $SCRIPT_PATH/latency_breakdown_analysis.py --log-path=$RESULT_DIR/api_server_${rate}.log > $RESULT_DIR/latency_breakdown_${rate}.log 2>&1
    sleep 3
done
