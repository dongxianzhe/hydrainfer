...
def create_conda_env(env_name, vllm_version):
    try:
        ...
        subprocess.run(f"conda run -n {env_name} --no-capture-output pip install {args.package}{args.extras}=={vllm_version}", shell=True, check=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--extras', type=str, default="", help="package extra requirements")
