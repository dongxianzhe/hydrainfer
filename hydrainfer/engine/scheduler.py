...
class BatchScheduler:
    def __init__(self, config: BatchSchedulerConfig, context: BatchSchedulerContext):
        method_map = {
            "STEP":STEPBatchScheduler, 
            "REQUEST":RequestLevelBatchScheduler, 
            "PREFILL":PrefillPrioritizeBatchScheduler, 
            "CONTINUOUS":ContinuousBatchScheduler, 
            "STALLFREE":STALLFREEBatchScheudler, 
        }
        method = os.environ.get("SCHEDULE_METHOD")
        assert method in method_map, f'invalid method {method}'
        self.scheduler = method_map[method](config, context)
...
class STEPBatchScheduler:
    ...
    def step(self) -> BatchRequest:
        ...
        modified_token_budget = int(self.token_budgets * (1 - batch_embed_images / self.image_budgets))
        ...
        for seq in fill_seqs:
            ...
            if batch_fill_tokens + n_tokens <= modified_token_budget:
                this_step.append(seq)
                batch_fill_tokens += n_tokens
            elif batch_fill_tokens < modified_token_budget and n_tokens > 1 and self.config.chunked_prefill and (isinstance(inst, TextFill) or isinstance(inst, ImageEmbedFill)): # if it is prefill and we can chunk part of it
                chunk_size = modified_token_budget - batch_fill_tokens
...
class RequestLevelBatchScheduler:
    ...
    def step(self) -> BatchRequest:
        if len(self.running) == 0:
            while not self.waiting.empty() and len(self.running) < self.config.max_running_requests:
...
class PrefillPrioritizeBatchScheduler:
    ...
    def step(self) -> BatchRequest:
        if not self.waiting.empty() and len(self.running) < self.config.max_running_requests:
            this_step: list[RequestControlBlock] = []
