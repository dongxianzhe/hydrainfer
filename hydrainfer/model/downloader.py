...
def download_hf_model(repo_id, revision=None, allow_patterns=None, cache_dir=None, convert_to_safetensors=False):
    """
    allow_patterns are file name patterns with which file will be downloaded from huggingface repo, seperated with comma. 
        eg. *.json,*.tiktoken,*.model,*.safetensors
    cache_dir is download path, None will download to default path 
    convert_to_safetensors will convert model weight with bin or pth format to safetensor format
    """
