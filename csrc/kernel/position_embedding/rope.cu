...
void apply_rotary_pos_emb(
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE(querys.scalar_type(), scalar_t, [&] {
    rotary_embedding_kernel<scalar_t>
        <<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>(
            static_cast<scalar_t*>(querys.data_ptr()),
            static_cast<scalar_t*>(keys.data_ptr()),
            static_cast<int*>(positions.data_ptr()),
            static_cast<scalar_t*>(cos_sin.data_ptr()),
            head_dim,
            rotary_dim,
            n_heads,
            n_kv_heads,
