...
public:
    ...
    LinearImpl(int in_features, int out_features, bool bias, const torch::TensorOptions& options){
        weight_ = register_parameter("weight", torch::empty({out_features, in_features}, options), false);
        if(bias) bias_ = register_parameter("bias", torch::empty({out_features}, options), false);
    }
